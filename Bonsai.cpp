...
Bonsai::Bonsai(uint64_t num_slots, uint64_t alp_size, uint8_t colls_bits) {
...
void Bonsai::show_stat(std::ostream& os) const {
...
HashValue Bonsai::hash_(const NodeID& node_id, uint64_t symbol) const {
...
bool Bonsai::get_child(NodeID& node_id, uint64_t symbol) const {
  if (alp_size_ <= symbol) {
...
bool Bonsai::add_child(NodeID& node_id, uint64_t symbol) {
  if (alp_size_ <= symbol) {
    std::cerr << "error : out-of-range symbol" << std::endl;
    exit(1);
  }
...
uint64_t Bonsai::find_ass_cbit_pos_(uint64_t pos, uint64_t& empty_pos) const {
...
uint64_t Bonsai::find_item_(uint64_t& pos, uint64_t quo) const {
...
uint64_t Bonsai::right_(uint64_t pos) const {
  return pos == num_slots_ - 1 ? 0 : pos + 1;
}

uint64_t Bonsai::left_(uint64_t pos) const {
  return pos == 0 ? num_slots_ - 1 : pos - 1;
}

// Copies a slot from the right slot except virgin bit information.
uint64_t Bonsai::copy_from_right_(uint64_t pos) {
  auto _pos = right_(pos);
  slots_[pos] = (slots_[_pos] & vbit_inv_mask_) | (get_vbit_(pos) << 2);
  return _pos;
}

uint64_t Bonsai::get_quo_(uint64_t pos) const {
  return slots_[pos] >> 3;
}

bool Bonsai::get_vbit_(uint64_t pos) const {
  return ((slots_[pos] >> 2) & 1U) == 1U;
}

bool Bonsai::get_cbit_(uint64_t pos) const {
  return ((slots_[pos] >> 1) & 1U) == 1U;
}

bool Bonsai::get_fbit_(uint64_t pos) const {
  return (slots_[pos] & 1U) == 1U;
}

void Bonsai::set_quo_(uint64_t pos, uint64_t quo) {
  slots_[pos] = (slots_[pos] & quo_inv_mask_) | (quo << 3);
}

void Bonsai::set_vbit_(uint64_t pos, bool bit) {
  slots_[pos] = (slots_[pos] & vbit_inv_mask_) | (bit << 2);
}

void Bonsai::set_cbit_(uint64_t pos, bool bit) {
  slots_[pos] = (slots_[pos] & cbit_inv_mask_) | (bit << 1);
}

void Bonsai::set_fbit_(uint64_t pos, bool bit) {
  slots_[pos] = (slots_[pos] & fbit_inv_mask_) | bit;
}

void Bonsai::update_slot_(uint64_t pos, uint64_t quo, bool vbit, bool cbit, bool fbit) {
